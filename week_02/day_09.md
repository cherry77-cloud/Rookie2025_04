...
- 构建 `fail` 数组
  ...
  - 初始化 `fail[0] = -1`，表示模式串的第一个字符没有前缀。
  - 对于每个位置 `i`，从 `fail[i-1]` 开始回退，直到找到匹配的前缀或回退到 `-1`。
  - 如果 `pattern[j + 1] == pattern[i]`，则 `fail[i] = j + 1`；否则，继续回退。

- 匹配过程
  
  - 初始化 `match = -1`，表示当前匹配的长度。
  - 遍历主字符串 `query`，对于每个字符 `query[i]`：
  - 如果 `pattern[match + 1] == query[i]`，则 `match++`。
  - 如果 `match == m - 1`，则表示找到一个匹配，记录匹配位置。
  - 如果匹配失败，则根据 `fail` 数组回退 `match`。
